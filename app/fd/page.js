...
import { TrendingUp, User, LogOut, Home, PieChart, Shield, Wallet, Star, ArrowUpRight, ArrowDownRight, Plus, X, RefreshCw, Edit3, Trash2, AlertTriangle, ArrowLeft, Lock, Mail, Palette, Send, ChevronRight, Calendar, Percent } from "lucide-react";
...
export default function FDPage() {
  ...
  const [formData, setFormData] = useState({ bankName: "", principal: "", interestRate: "", tenure: "", startDate: "" });
  ...
  useEffect(() => { if (portfolio?.fd) setFds(portfolio.fd); }, [portfolio?.fd]);
  ...
  const handleAdd = () => { setEditingFd(null); setFormData({ bankName: "", principal: "", interestRate: "", tenure: "", startDate: "" }); setShowModal(true); };
  const handleEdit = (fd) => { setEditingFd(fd); setFormData({ bankName: fd.bankName, principal: fd.principal.toString(), interestRate: fd.interestRate.toString(), tenure: fd.tenure.toString(), startDate: fd.startDate }); setShowModal(true); };
  const handleDeleteClick = (fd) => { setFdToDelete(fd); setShowDeleteModal(true); };
  
  const confirmDelete = async () => {
    if (fdToDelete) {
      const updated = fds.filter(f => f.id !== fdToDelete.id);
      await saveData('fd', updated);
      triggerSync();
      setShowDeleteModal(false); setFdToDelete(null); showToast("🗑️ FD deleted.", "error");
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    let updated;
    if (editingFd) {
      updated = fds.map(f => f.id === editingFd.id ? fdData : f);
    } else {
      updated = [...fds, fdData];
    }
    await saveData('fd', updated);
    triggerSync();
    showToast(editingFd ? "✅ FD updated!" : "🎉 FD added!");
    setShowModal(false);
  };
  ...
  if (!user) return (
    <main className={`min-h-screen flex items-center justify-center p-4 font-sans ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} p-5 rounded-2xl shadow-lg w-full max-w-sm border text-center`}>
        <p className={textMuted}>Please login to view FDs</p>
      ...
      <div className="w-full md:max-w-[420px] h-full md:h-screen relative flex flex-col md:shadow-2xl md:border-x md:border-gray-200">
        ...
        <div className={`${isDark ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} backdrop-blur-md z-20 flex-shrink-0 border-b sticky top-0`}>
          <header className="px-3 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <Link href="/" className="bg-amber-600 p-1.5 rounded-lg"><TrendingUp size={18} className="text-white"/></Link>
              <div><h1 className="text-base font-bold leading-none">Fixed Deposits</h1><div className="flex items-center gap-1.5 mt-0.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span><span className={`text-[10px] font-medium truncate max-w-[120px] ${textMuted}`}>{user.userId}</span></div></div>
            </div>
        ...
        <div className={`flex-1 overflow-y-auto pb-20 px-3 pt-3 space-y-3 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
          <div className="flex items-center justify-between mb-2">
            <h2 className={`text-lg font-bold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>My FDs ({fds.length})</h2>
            <button onClick={handleAdd} className="bg-amber-600 hover:bg-amber-700 text-white px-3 py-2 rounded-full font-bold text-xs flex items-center gap-1 shadow-sm transition"><Plus size={14} /> Add FD</button>
          </div>
          
          {fds.length === 0 ? (
            <div className="text-center py-10 select-none h-full flex flex-col items-center justify-center">
              <div className="bg-amber-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3"><Wallet size={32} className="text-amber-600" /></div>
              <p className={textMuted}>No FDs added yet</p>
              <button onClick={handleAdd} className="mt-3 text-amber-600 text-sm font-bold hover:underline">+ Add your first FD</button>
            </div>
          ) : (
            <div className="space-y-3 pb-24">
              {fds.map((fd) => {
                const gain = fd.maturityAmount - fd.principal;
                const gainPct = ((gain / fd.principal) * 100).toFixed(1);
                return (
                  <div key={fd.id} className={`${bgCard} rounded-2xl overflow-hidden shadow-sm border transition-all duration-300`}>
                    <div className="p-4 pb-2 flex justify-between items-start">
                      <div className="pr-8"><span className={`text-[10px] ${textMuted} font-medium flex items-center gap-1`}><Calendar size={10}/> {fd.startDate}</span><h3 className={`font-bold text-sm leading-tight mt-1 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{fd.bankName}</h3></div>
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(fd)} className="p-1.5 text-amber-500 hover:bg-amber-50 dark:hover:bg-gray-700 rounded-lg transition"><Edit3 size={16} /></button>
                        <button onClick={() => handleDeleteClick(fd)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-gray-700 rounded-lg transition"><Trash2 size={16} /></button>
                      </div>
                    </div>
                    <div className={`grid grid-cols-3 gap-1 px-4 py-3 ${isDark ? 'bg-gray-700/50' : 'bg-gray-50/50'} border-b ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
                      <div className="text-center"><div className={`text-[9px] ${textMuted} uppercase font-semibold`}>Principal</div><div className={`font-bold text-xs mt-0.5 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>₹{(fd.principal/1000).toFixed(1)}K</div></div>
                      <div className="text-center"><div className={`text-[9px] ${textMuted} uppercase font-semibold`}>Rate</div><div className={`font-bold text-xs mt-0.5 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{fd.interestRate}%</div></div>
                      <div className="text-center"><div className={`text-[9px] ${textMuted} uppercase font-semibold`}>Tenure</div><div className={`font-bold text-xs mt-0.5 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{fd.tenure} Yrs</div></div>
                    </div>
                    <div className="flex items-center justify-between px-4 py-2 bg-amber-50/50 dark:bg-amber-900/10">
                      <div className="flex items-center gap-1 font-bold text-xs text-amber-700 dark:text-amber-400"><Percent size={12} />Gain: +{gainPct}%</div>
                      <div className="text-right"><div className="text-[9px] text-gray-500 uppercase">Maturity</div><div className="font-bold text-sm text-amber-800 dark:text-amber-300">₹{(fd.maturityAmount/1000).toFixed(2)}K</div></div>
                    </div>
                  </div>
                );
              })}
        ...
        <nav className={`${isDark ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} backdrop-blur-md border-t fixed bottom-0 w-full md:max-w-[420px] flex justify-around py-2.5 pb-4 z-20 select-none`}>
          <Link href="/" className="flex flex-col items-center gap-0.5 text-gray-400 hover:text-amber-600 transition"><Home size={18}/><span className="text-[10px] font-medium">Home</span></Link>
          ...
          <Link href="/fd" className="flex flex-col items-center gap-0.5 text-amber-600"><Wallet size={18}/><span className="text-[10px] font-bold">FD</span></Link>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setShowModal(false)}>
            <div className={`${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} w-full md:max-w-[400px] md:rounded-2xl rounded-t-2xl p-6 shadow-2xl`} onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-6"><h3 className="text-lg font-bold">{editingFd ? 'Edit FD' : 'Add New FD'}</h3><button onClick={() => setShowModal(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition"><X size={20} className={textMuted}/></button></div>
              <form onSubmit={handleSave} className="space-y-4">
                <div><label className={`block text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Bank Name</label><input type="text" value={formData.bankName} onChange={(e) => setFormData({...formData, bankName: e.target.value})} placeholder="e.g. SBI FD" required className={`w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-amber-500/30 transition ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={`block text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Principal (₹)</label><input type="number" value={formData.principal} onChange={(e) => setFormData({...formData, principal: e.target.value})} placeholder="50000" required step="0.01" className={`w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-amber-500/30 transition ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`} /></div>
                  <div><label className={`block text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Rate (%)</label><input type="number" value={formData.interestRate} onChange={(e) => setFormData({...formData, interestRate: e.target.value})} placeholder="7.5" required step="0.01" className={`w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-amber-500/30 transition ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`} /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={`block text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Tenure (Years)</label><input type="number" value={formData.tenure} onChange={(e) => setFormData({...formData, tenure: e.target.value})} placeholder="5" required step="0.1" className={`w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-amber-500/30 transition ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`} /></div>
                  <div><label className={`block text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-1`}>Start Date</label><input type="date" value={formData.startDate} onChange={(e) => setFormData({...formData, startDate: e.target.value})} required className={`w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-amber-500/30 transition ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`} /></div>
                </div>
                {formData.principal && formData.interestRate && formData.tenure && (
                  <div className={`${isDark ? 'bg-amber-900/30 border-amber-800' : 'bg-amber-50 border-amber-100'} border rounded-xl p-3`}>
                    <div className={`text-xs ${isDark ? 'text-amber-400' : 'text-amber-600'} font-semibold`}>Calculated Maturity</div>
                    <div className={`text-lg font-bold ${isDark ? 'text-amber-300' : 'text-amber-700'}`}>₹{(parseFloat(formData.principal) + (parseFloat(formData.principal) * parseFloat(formData.interestRate) * parseFloat(formData.tenure) / 100)).toLocaleString('en-IN')}</div>
                  </div>
                )}
                <div className="flex gap-3 pt-2"><button type="button" onClick={() => setShowModal(false)} className={`flex-1 px-4 py-3 rounded-xl text-sm font-bold transition ${isDark ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Cancel</button><button type="submit" className="flex-1 px-4 py-3 bg-amber-600 text-white rounded-xl text-sm font-bold hover:bg-amber-700 transition shadow-lg shadow-amber-200">{editingFd ? 'Update' : 'Add'} FD</button></div>
              </form>
            </div>
          </div>
        )}
        ...
        {showDeleteModal && fdToDelete && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setShowDeleteModal(false)}>
            <div className={`${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} w-full md:max-w-[380px] md:rounded-2xl rounded-t-2xl p-6 shadow-2xl`} onClick={e => e.stopPropagation()}>
              ...
              <h3 className={`text-xl font-bold text-center mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Delete FD?</h3>
              <p className={`text-sm text-center mb-6 ${textMuted}`}>Kya aap sach mein <span className={`font-bold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>"{fdToDelete.bankName}"</span> ko delete karna chahte hain?</p>
